...
describe('PerformanceMonitor', () => {
  ...
  describe('performance alerts', () => {
    ...
    it('should warn for slow responses (>200ms)', () => {
      ...
      expect(console.warn).toHaveBeenCalledWith(
        expect.stringContaining(
          '[PERFORMANCE ALERT] Slow API response: /api/test took 250ms'
        )
      );
    });
    ...
    it('should use configurable database query threshold', () => {
      ...
      expect(console.warn).toHaveBeenCalledWith(
        expect.stringContaining(
          '[PERFORMANCE ALERT] Slow database query: /api/test query took 40ms (threshold: 30ms)'
        )
      );
    });
    ...
    it('should use default threshold when not configured', () => {
      ...
      expect(console.warn).toHaveBeenCalledWith(
        expect.stringContaining(
          '[PERFORMANCE ALERT] Slow database query: /api/test query took 60ms (threshold: 50ms)'
        )
      );
    });
    ...
    it('should error for very slow responses (>1000ms)', () => {
      ...
      expect(console.error).toHaveBeenCalledWith(
        expect.stringContaining(
          '[CRITICAL PERFORMANCE] Very slow API response: /api/test took 1500ms'
        )
      );
    });
