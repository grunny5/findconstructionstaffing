...
-- 3. Verifies count BEFORE commit (ensures atomic rollback on failure)
-- 4. Uses ON CONFLICT to be safe for re-running (upsert pattern)

-- Wrap entire migration in DO block for automatic transaction + rollback
DO $$
DECLARE
  trade_count INTEGER;
BEGIN
  ...
  -- Verify trade count BEFORE committing
  -- If this check fails, the entire transaction will rollback automatically
  SELECT COUNT(*) INTO trade_count FROM trades;

  IF trade_count != 57 THEN
    RAISE EXCEPTION 'Migration verification failed: Expected 57 trades, found %. Rolling back.', trade_count;
  END IF;

  -- Success message
  RAISE NOTICE 'Migration completed successfully: Standardized to % trades', trade_count;

  -- Transaction will auto-commit here if no exception was raised
END $$;
