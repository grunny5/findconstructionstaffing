...
async function testScenario(scenario) {
  ...
  if (results.length > 0) {
    ...
    if (successfulResults.length > 0) {
      ...
      const medianResponseTime = responseTimes.sort((a, b) => a - b)[
        Math.floor(responseTimes.length / 2)
      ];

      const avgDataSize =
        successfulResults.reduce((sum, r) => sum + r.dataSize, 0) /
        successfulResults.length;
      const avgResultCount =
        successfulResults.reduce((sum, r) => sum + r.resultCount, 0) /
        successfulResults.length;
      ...
      console.log(
        `      - Average: ${avgResponseTime.toFixed(2)}ms ${avgResponseTime > TARGET_RESPONSE_TIME ? '⚠️ ABOVE TARGET' : '✅'}`
      );
      console.log(`      - Median: ${medianResponseTime}ms`);
      console.log(`      - Min: ${minResponseTime}ms`);
      console.log(`      - Max: ${maxResponseTime}ms`);
      console.log(
        `   📦 Average response size: ${(avgDataSize / 1024).toFixed(2)} KB`
      );
      console.log(
        `   📊 Results: ${avgResultCount.toFixed(1)} items (${totalCount} total)`
      );
      ...
      console.log(
        `   💾 Caching: ${cacheControl || 'Not set'} ${hasEtag ? '(ETag ✅)' : '(No ETag ❌)'}`
      );
...
async function runPerformanceTests() {
  ...
  if (allResults.length > 0) {
    // Overall statistics
    const avgOverall =
      allResults.reduce((sum, r) => sum + r.avgResponseTime, 0) /
      allResults.length;
    ...
    console.log('\n📈 Overall Performance:');
    console.log(
      `   Average Response Time: ${avgOverall.toFixed(2)}ms ${avgOverall > TARGET_RESPONSE_TIME ? '⚠️' : '✅'}`
    );
    ...
    allResults.forEach((result) => {
      const name =
        result.scenario.length > 37
          ? result.scenario.substring(0, 37) + '...'
          : result.scenario;
      console.log(
        '   ' +
          name.padEnd(40) +
    ...
    const slowQueries = allResults.filter(
      (r) => r.avgResponseTime > TARGET_RESPONSE_TIME
    );
    ...
    if (slowQueries.some((r) => r.scenario.includes('All filters'))) {
      ...
      console.log(
        '   - Consider implementing cursor-based pagination for large offsets'
      );
    }
