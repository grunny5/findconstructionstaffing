...
async function verifyRelationships() {
  ...
  try {
    ...
    if (agencyError) {
      ...
      console.log('   Will verify relationships using alternative methods...\n');
    } else {
    ...
    if (testDataCreated && testAgency) {
      ...
      if (testAgency && testTrade && testRegion) {
        ...
        if (!tradeRelError && !regionRelError) {
          ...
          console.log('⚠️  Could not create test relationships (RLS may be blocking)');
        }
      }
    }
    ...
    if (!testDataCreated) {
      ...
      if (!joinError) {
        ...
        console.log('   Junction tables and foreign keys are properly configured');
      } else {
    ...
    if (testDataCreated) {
      ...
      if (testTrade) {
        cleanupOperations.push(
          supabase.from('trades').delete().eq('slug', 'test-trade-relations')
            .then(() => console.log('   ✓ Cleaned up test trade'))
            .catch(err => console.log(`   ⚠️  Failed to clean up test trade: ${err.message}`))
        );
      }
      
      if (testRegion) {
        cleanupOperations.push(
          supabase.from('regions').delete().eq('slug', 'test-region-tx')
            .then(() => console.log('   ✓ Cleaned up test region'))
            .catch(err => console.log(`   ⚠️  Failed to clean up test region: ${err.message}`))
        );
      }
