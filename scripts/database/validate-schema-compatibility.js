...
if (!fs.existsSync(mockDataPath)) {
  ...
  console.error(
    '   This file is required for schema compatibility validation.'
  );
  process.exit(1);
}
...
try {
  ...
  const mockAgenciesMatch = mockDataContent.match(
    /export const mockAgencies = \[([\s\S]*?)\];/
  );
  ...
  expectedFields.forEach((field) => {
    if (!mockFields.has(field)) {
      console.warn(
        `⚠️  Expected field '${field}' not found in parsed mock data`
      );
      mockFields.add(field);
...
console.log('🔍 Mock Data Fields Found:');
mockFields.forEach((field) => {
  const dbColumn = schemaMapping[field] || '❌ UNMAPPED';
  const type = fieldTypes[field] || 'unknown';
  console.log(`   - ${field} (${type}) → ${dbColumn}`);
});

console.log('\n📋 Database Fields Status:');
console.log('\n✅ Mapped from Mock Data:');
...
async function testDatabaseInsert() {
  console.log('\n🧪 Testing Database Insert Capability...\n');
...
const report = {
  ...
  transformations: [
    'name → slug (using createSlug)',
    'trades[] → agency_trades junction',
    'regions[] → agency_regions junction',
  ],
