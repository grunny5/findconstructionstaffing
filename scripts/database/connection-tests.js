...
async function runConnectionTests() {
  ...
  try {
    ...
    const promises = Array(10)
      .fill(null)
      .map((_, i) => supabase.from('agencies').select('id').limit(1));
  ...
  try {
    ...
    if (!error) {
      ...
      results.passed++;
      results.tests.push({
        name: 'Request Headers Validation',
        status: 'PASS',
      });
    } else {
      console.log(`❌ FAIL - ${error.message}`);
  ...
  if (performanceTests.length > 0) {
    const avgDuration =
      performanceTests.reduce((sum, t) => sum + t.duration, 0) /
  ...
  if (results.failed === 0) {
    ...
    console.log(
      'Review the failures and ensure your connection is properly configured.'
    );
  }
}
