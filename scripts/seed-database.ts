...
function validateEnvironment(): { url: string; key: string } {
  ...
  if (!key.includes('.') || key.split('.').length !== 3) {
    ...
    log.info('The service role key should be a JWT token with three parts separated by dots.');
    process.exit(1);
  }
...
async function testConnection(supabase: ReturnType<typeof createSupabaseClient>): Promise<boolean> {
  try {
...
async function seedTrades(
  ...
  try {
    ...
    for (let i = 0; i < uniqueTrades.length; i += batchSize) {
      ...
      if (fetchError) {
        throw new Error(`Failed to fetch existing trades: ${fetchError.message}`);
      }
    ...
    log.info(`Created: ${created}, Skipped: ${skipped}, Total: ${tradeIdMap.size}`);
  ...
  try {
    ...
    for (let i = 0; i < uniqueStates.length; i += batchSize) {
      ...
      const stateRecords = batch
        ...
        .filter((record): record is { name: string; code: string } => record !== null);
      ...
      if (fetchError) {
        throw new Error(`Failed to fetch existing regions: ${fetchError.message}`);
      }
    ...
    log.info(`Created: ${created}, Skipped: ${skipped}, Invalid: ${invalid}, Total: ${regionIdMap.size}`);
...
async function seedAgencies(
  ...
  try {
    ...
    for (let i = 0; i < mockAgencies.length; i += batchSize) {
      ...
      if (fetchError) {
        throw new Error(`Failed to fetch existing agencies: ${fetchError.message}`);
      }
    ...
    log.info(`Created: ${created}, Skipped: ${skipped}, Total: ${agencyIdMap.size}`);
  ...
  try {
    ...
    for (const agency of mockAgencies) {
      ...
      for (const tradeName of agency.trades) {
        ...
        if (!tradeId) {
          log.warning(`Trade ID not found for: ${tradeName} (agency: ${agency.name})`);
          errors++;
          continue;
        }
  ...
  try {
    ...
    for (const agency of mockAgencies) {
      ...
      for (const stateName of agency.regions) {
        ...
        if (!stateCode) {
          log.warning(`State code not found for: ${stateName} (agency: ${agency.name})`);
          errors++;
          continue;
        }
        ...
        if (!regionId) {
          log.warning(`Region ID not found for state code: ${stateCode} (${stateName}, agency: ${agency.name})`);
          errors++;
          continue;
        }
...
async function resetDatabase(
  ...
  try {
    ...
    if (agencyTradeError) {
      throw new Error(`Failed to delete agency-trade relationships: ${agencyTradeError.message}`);
    }
    ...
    if (agencyRegionError) {
      throw new Error(`Failed to delete agency-region relationships: ${agencyRegionError.message}`);
    }
    ...
    if (error instanceof Error) {
      ...
      log.warning('Database may be in an inconsistent state. Manual cleanup may be required.');
    }
    throw error;
  }
}
