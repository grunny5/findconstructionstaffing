...
console.log(
  `Key length: ${envVars.NEXT_PUBLIC_SUPABASE_ANON_KEY ? envVars.NEXT_PUBLIC_SUPABASE_ANON_KEY.length : 0} characters`
);
console.log(
  `Key preview: ${envVars.NEXT_PUBLIC_SUPABASE_ANON_KEY ? envVars.NEXT_PUBLIC_SUPABASE_ANON_KEY.substring(0, 50) + '...' : 'NOT SET'}\n`
);
...
try {
  ...
  if (payload.exp) {
    ...
    console.log(
      `- Expires: ${expiryDate.toISOString()} ${isExpired ? '❌ EXPIRED' : '✅ Valid'}`
    );

    if (isExpired) {
      console.log('\n⚠️  WARNING: Your JWT token has expired!');
      console.log(
        '   Please generate a new anon key from your Supabase dashboard.'
      );
    }
  } else {
  ...
  console.log(
    '   - The environment variable contains extra characters or formatting'
  );
}
    ...
    const req = https.request(options, (res) => {
      ...
      res.on('end', () => {
        ...
        if (res.statusCode === 401) {
          ...
          console.log(
            '  ⚠️  Not Found: Resource does not exist (this may be expected)'
          );
          ...
          try {
            ...
            if (responseData.error) {
              ...
              console.log(
                `  ❌ Unauthorized response: ${responseData.message}`
              );
              hasErrors = true;
            } else if (
...
(async () => {
  ...
  if (hasErrors) {
    ...
    console.log(
      '1. If you see 401 errors, verify your anon key in Supabase dashboard'
    );
    console.log(
      "2. If the URL/Key don't match, check you're using the right project"
    );
