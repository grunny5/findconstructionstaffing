...
async function testRLSInsert() {
  ...
  if (agencyError) {
    ...
    const isRLSViolation =
      agencyError.code === '42501' || // PostgreSQL insufficient_privilege error
      agencyError.code === 'PGRST301' || // PostgREST RLS violation
      (agencyError.message &&
        agencyError.message.toLowerCase().includes('row-level security')) ||
      (agencyError.message &&
        agencyError.message.toLowerCase().includes('policy'));
    ...
    console.log(
      '❌ RLS NOT WORKING: Insert succeeded (should have been blocked)'
    );
  }
  ...
  if (tradeError) {
    // Check for RLS violations using error code or message patterns
    const isRLSViolation =
      tradeError.code === '42501' || // PostgreSQL insufficient_privilege error
      tradeError.code === 'PGRST301' || // PostgREST RLS violation
      (tradeError.message &&
        tradeError.message.toLowerCase().includes('row-level security')) ||
      (tradeError.message &&
        tradeError.message.toLowerCase().includes('policy'));
