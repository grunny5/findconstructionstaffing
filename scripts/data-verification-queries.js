...
async function runVerificationQueries() {
  ...
  try {
    ...
    if (agencyTradeError) {
      throw new Error(
        `Failed to count agency-trade relationships: ${agencyTradeError.message}`
      );
    }
    ...
    if (agencyRegionError) {
      throw new Error(
        `Failed to count agency-region relationships: ${agencyRegionError.message}`
      );
    }
    ...
    if (agenciesWithTrades) {
      ...
      agenciesWithTrades.forEach((agency) => {
        ...
        if (agency.agency_trades) {
          if (
            Array.isArray(agency.agency_trades) &&
            agency.agency_trades.length > 0
          ) {
            // Handle array format
    ...
    if (agenciesWithRegions) {
      ...
      agenciesWithRegions.forEach((agency) => {
        ...
        if (agency.agency_regions) {
          if (
            Array.isArray(agency.agency_regions) &&
            agency.agency_regions.length > 0
          ) {
            // Handle array format
  ...
  try {
    ...
    if (linkedTradesError) {
      throw new Error(
        `Failed to get linked trades: ${linkedTradesError.message}`
      );
    }
    ...
    if (linkedRegionsError) {
      throw new Error(
        `Failed to get linked regions: ${linkedRegionsError.message}`
      );
    }
  ...
  try {
    ...
    if (incompleteError) {
      throw new Error(
        `Failed to check incomplete agencies: ${incompleteError.message}`
      );
    }

    if (incompleteAgencies && incompleteAgencies.length > 0) {
      console.log(
        `   ⚠️  ${incompleteAgencies.length} agencies with missing required fields`
      );
    } else {
  ...
  const queriesPath = path.join(
    ...
    '..',
    'docs',
    'migration-verification-queries.sql'
  );
  let sqlContent =
