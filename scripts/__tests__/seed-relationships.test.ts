import { createAgencyTradeRelationships, createAgencyRegionRelationships, createStateMapping } from '../seed-database';
...
describe('Agency-Trade Relationship Functions', () => {
  describe('createAgencyTradeRelationships', () => {
    ...
    it('should handle database errors gracefully', async () => {
      ...
      const agencyIdMap = new Map([['Industrial Staffing Solutions', 'agency-1']]);
      ...
      await expect(
        createAgencyTradeRelationships(mockClient as any, agencyIdMap, tradeIdMap)
      ).rejects.toThrow('Failed to fetch existing relationships: Database connection failed');
    });
  ...
  describe('createAgencyRegionRelationships', () => {
    ...
    it('should handle state name to code mapping correctly', async () => {
      ...
      const agency = mockAgencies.find(a => a.name === 'Industrial Staffing Solutions');
      ...
      const hasTexas = insertedRelationships.some(r => r.region_id === 'region-TX');
      const hasLouisiana = insertedRelationships.some(r => r.region_id === 'region-LA');
