  ...
  describe('seedAgencies', () => {
    ...
    it('should skip existing agencies', async () => {
      ...
      const otherAgencies = mockAgencies.filter(
        a => a.name !== 'Industrial Staffing Solutions' && a.name !== 'TradePower Recruiting'
      );
    ...
    it('should apply correct data transformations', async () => {
      ...
      allInsertedData.forEach((insertedAgency: any, index: number) => {
        const originalAgency = mockAgencies.find(a => a.name === insertedAgency.name);
        ...
        expect(insertedAgency.offers_per_diem).toBe(originalAgency?.offers_per_diem);
    ...
    it('should preserve all agency fields correctly', async () => {
      ...
      const expectedFields = [
        ...
        'phone', 'email', 'is_claimed', 'is_active', 'offers_per_diem',
        'is_union', 'founded_year', 'employee_count', 'headquarters',
