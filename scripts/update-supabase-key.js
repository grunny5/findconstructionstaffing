...
rl.question('Paste your anon public key here: ', (anonKey) => {
  ...
  if (!trimmedKey || !isValidJWT) {
    ...
    console.error('   The anon key should be a JWT token with 3 parts separated by dots.');
    console.error('   Example format: xxxxx.yyyyy.zzzzz');
    console.error('   Each part should only contain base64url characters (A-Z, a-z, 0-9, _, -)');
    rl.close();
    process.exit(1);
  }
  ...
  try {
    ...
    if (!payload.role || !payload.ref) {
      ...
      console.error('   Missing expected fields (role, ref) in the JWT payload.');
    }
  } catch (e) {
...
NEXT_PUBLIC_SUPABASE_ANON_KEY=${trimmedKey}`;
  ...
  try {
    ...
    console.error('   Please check file permissions and ensure the directory is writable.');
    rl.close();
    process.exit(1);
  }
