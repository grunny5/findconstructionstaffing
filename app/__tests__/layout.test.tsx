...
describe('Root Layout - Font Configuration', () => {
  ...
  describe('Bebas Neue Configuration', () => {
    it('should configure Bebas Neue with weight 400', () => {
      expect(layoutContent).toMatch(/Bebas_Neue\(\{[\s\S]*?weight:\s*['"]400['"]/);
    });

    it('should configure Bebas Neue with display swap', () => {
      expect(layoutContent).toMatch(/Bebas_Neue\(\{[\s\S]*?display:\s*['"]swap['"]/);
    });

    it('should configure Bebas Neue with latin subset', () => {
      expect(layoutContent).toMatch(/Bebas_Neue\(\{[\s\S]*?subsets:\s*\[['"]latin['"]\]/);
    });

    it('should create CSS variable --font-bebas-neue', () => {
      expect(layoutContent).toMatch(/Bebas_Neue\(\{[\s\S]*?variable:\s*['"]--font-bebas-neue['"]/);
    });
  });

  describe('Barlow Configuration', () => {
    it('should configure Barlow with multiple weights', () => {
      expect(layoutContent).toMatch(/Barlow\(\{[\s\S]*?weight:\s*\[['"]400['"],\s*['"]500['"],\s*['"]600['"],\s*['"]700['"]\]/);
    });

    it('should configure Barlow with display swap', () => {
      expect(layoutContent).toMatch(/Barlow\(\{[\s\S]*?display:\s*['"]swap['"]/);
    });

    it('should configure Barlow with latin subset', () => {
      expect(layoutContent).toMatch(/Barlow\(\{[\s\S]*?subsets:\s*\[['"]latin['"]\]/);
    });

    it('should create CSS variable --font-barlow', () => {
      expect(layoutContent).toMatch(/Barlow\(\{[\s\S]*?variable:\s*['"]--font-barlow['"]/);
    });
  });

  describe('Libre Barcode 39 Text Configuration', () => {
    it('should configure Libre Barcode with weight 400', () => {
      expect(layoutContent).toMatch(/Libre_Barcode_39_Text\(\{[\s\S]*?weight:\s*['"]400['"]/);
    });

    it('should configure Libre Barcode with display swap', () => {
      expect(layoutContent).toMatch(/Libre_Barcode_39_Text\(\{[\s\S]*?display:\s*['"]swap['"]/);
    });

    it('should configure Libre Barcode with latin subset', () => {
      expect(layoutContent).toMatch(/Libre_Barcode_39_Text\(\{[\s\S]*?subsets:\s*\[['"]latin['"]\]/);
    });

    it('should create CSS variable --font-libre-barcode', () => {
      expect(layoutContent).toMatch(/Libre_Barcode_39_Text\(\{[\s\S]*?variable:\s*['"]--font-libre-barcode['"]/);
    });
  });
